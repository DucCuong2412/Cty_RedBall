...
public class YuruyenMerdSol : MonoBehaviour
{
	...
	private void FixedUpdate()
	{
		for (int i = 0; i < 16; i++)
		{
			...
			{
				if (gameObject.transform.position.x < Sol.x)
				{
			...
			else if (gameObject.transform.position.x > Sag.x)
			{
				gameObject.transform.position = Sag;
				if (gameObject.transform.rotation.eulerAngles.z < 359f)
				{
					gameObject.transform.Rotate(0f, 0f, 5f);
				}
				if (gameObject.transform.rotation.eulerAngles.z <= 0f || gameObject.transform.rotation.eulerAngles.z >= 360f)
				{
